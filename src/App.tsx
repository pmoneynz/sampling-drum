...
function App() {
  ...
  const [progressStage, setProgressStage] = useState<'starting' | 'extracting' | 'processing' | 'complete'>('starting');
  ...
  const handleVideoLoad = async (info: YouTubeVideoInfo, quality: VideoQuality) => {
    ...
    setProgressMessage('Initializing video extraction...');
    ...
    try {
      ...
      // Extract video/audio with progress tracking
      const extraction = await youtubeService.extractVideo(
        `https://www.youtube.com/watch?v=${info.id}`,
        quality,
        {
          onProgress: (data) => {
            setProgressStage(data.stage as any);
            setAudioProgress(data.audioProgress);
            setVideoProgress(data.videoProgress);
            setProgressMessage(data.message);
            setProgressSpeed(data.speed);
            setProgressEta(data.eta);
          },
          onError: (errorData) => {
            setError(errorData.error);
            setErrorType(errorData.errorType);
            setErrorSuggestion(errorData.suggestion);
            setIsLoading(false);
          },
          onComplete: () => {
            setProgressStage('complete');
            setProgressMessage('Video extraction completed successfully!');
          }
        }
      );
      ...
      setError(error?.userMessage || error?.message || 'Failed to load video');
      setErrorType(error?.type || 'UNKNOWN_ERROR');
      setErrorSuggestion(error?.suggestion || 'Please try a different video');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleRetry = () => {
    if (videoInfo) {
      handleVideoLoad(videoInfo, currentQuality);
    }
  };
  ...
  const handleCancelLoading = () => {
    setIsLoading(false);
    youtubeService.disconnectSocket();
  };
